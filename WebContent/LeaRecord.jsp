...
<body>
	<div class="off-canvas-wrapper">
				...
				<div id="app" style="height: 590px">
					...
					<div class="panel panel-primary" STYLE="width: 100%; height: 20%;">
						<div class="panel-heading">
							<h3 class="panel-title" STYLE="margin-left: 730px">学习记录管理</h3>
						</div>

						<div STYLE="width: 20%; height: 100%; margin-left: 10%;">
							<form>
								<label style="margin-top: 20px;"> <input type="text"
									placeholder="用户名" class="form-control" v-model="keywords"
									id="search" v-focus v-color="'green'">
								</label> <input type="button" value="查询" class="btn btn-primary"
									onclick="userqueryName()" />
							</form>
						</div>
					...
					<table class="table" style="margin-top: 20px">
						...
						<s:iterator var="leaRecord" value="leaRecordList" status="st">
							<tr>
								<td><s:property value="#st.index+1" /></td>
								<td><s:property value="#leaRecord.user.userAccount" /></td>
								<td><s:property value="#leaRecord.playTime" /></td>
								<td><fmt:formatDate pattern="yyyy-MM-dd" value="${times}" />
								</td>
								<td><s:property value="#leaRecord.totalLength" /></td>
								<td><s:if test="#leaRecord.finish==0">未完成</s:if> <s:if
										test="#leaRecord.finish==1">完成</s:if></td>

								<td><s:property value="#leaRecord.batch.batchName" /></td>
								<td><s:property value="#leaRecord.courseware.cwName" /></td>
								<td><a
									href="Leadelete?ids=<s:property value="#leaRecord.id"/>">删除</a></td>
							</tr>
						</s:iterator>
					</table>
				</div>
		...
		<script>
			...
			function userqueryName() {
				var form = document.forms[0];
				var search = encodeURI(encodeURI(document
						.getElementById("search").value));
				form.action = "userqueryName" + "?userAccount=" + search;
				form.method = "post";
				form.submit();
			}
		</script>
